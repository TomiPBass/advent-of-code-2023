const input = `Game 1: 4 blue, 7 red, 5 green; 3 blue, 4 red, 16 green; 3 red, 11 green
...
const testingInput = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
const rulesKeys = Object.keys(rules) as (keyof typeof rules)[];
...
const parseGameInput = (input: string): Game[] => {
    const arrayOfGameStrings = input.split('\n');
    const arrayOfGames = [] as Game[];
    arrayOfGameStrings.forEach((gameString) => {
        let game = {
            sets: [],
        } as Game;
        const gameTitle = gameString.split(':')[0];
        const gameSets = gameString.split(':')[1];
        const gameId = +gameTitle.split(' ')[1];
        game.id = gameId;
        const gameSetsArray = gameSets.split(';');
        for (const set of gameSetsArray) {
            let currentSet: Set;
            const subsetArray = set.split(',');
            for (const subset of subsetArray) {
                const trimmedSubset = subset.trim();
                const color = trimmedSubset.split(' ')[1];
                const amount = +trimmedSubset.split(' ')[0];
                currentSet = {
                    ...currentSet,
                    [color]: amount
                }
...
const sumAllTheIdsOfPossibleGames = (input: string, rules: Set) => {
    // Parse the input into a clear data structure
    const parsedInput = parseGameInput(input);
    const succesfulGamesIds =  [] as number[];

    // Loop through the games
    parsedInput.forEach((game) => {
        let isSuccessfulGame = true;
    ...
    return succesfulGamesIds.reduce((acc, curr) => acc + curr, 0);
}

const sumOfAllThePowersOfSets = (input: string) => {
    const parsedInput = parseGameInput(input);
    const allSetsPowers = [] as number[];

    // Loop through the games
    parsedInput.forEach((game) => {
        let powerOfTheSets = 0;
        const maximums: Record<typeof rulesKeys[number], number> = {
            blue: 0,
            red: 0,
            green: 0,
        }
        ...
        for (const maximum in maximums) {
            if (powerOfTheSets === 0) {
                powerOfTheSets = maximums[maximum as keyof typeof maximums];
            } else {
                powerOfTheSets *= maximums[maximum as keyof typeof maximums];
            }
        }
    ...
    return allSetsPowers.reduce((acc, curr) => acc + curr, 0);
}
